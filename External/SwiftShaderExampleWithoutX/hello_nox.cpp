...
bool CreateEGLSurface(EGLDisplay eglDisplay, EGLConfig eglConfig, EGLSurface& eglSurface)
{
	...
	*/
	...
	if (!TestEGLError("eglCreatePbufferSurface"))
	{
		return false;
	}
...
int main(int /*argc*/, char **/*argv*/)
{
	...
	FILE *output;
	int desiredWidth = pbufferAttribs[1];
	int desiredHeight = pbufferAttribs[3];
	// EGL variables
	...
	buffer = (unsigned char*)malloc(desiredWidth * desiredHeight * 4);
	
	// Copy entire screen
	glReadPixels(0, 0, desiredWidth, desiredHeight, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
	
	// Write all pixels to file
	output = fopen("hello.data", "wb");
	if(output){
		fwrite(buffer, 1, desiredWidth * desiredHeight * 4, output);
		fclose(output);
	} else {
		fprintf(stderr, "Failed to open file hello.data for writing!\n");
	}
